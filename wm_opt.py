...
import numpy as np
import pickle
from util import cvt2Image, sparse_tuple_from, invert, transpose, cvt2raw, show
...
with open(f'attack_result/wm_grad/{wm0_path}', 'rb') as f:
    (_, wm0_mask, _, wm0_img, _, _, _, _, _, _, _) = pickle.load(f)

# small sample set
n_img = 200
input_img, len_x = wm0_img[:n_img], len_x[:n_img]
gt_txt, target_txt = gt_txt[:n_img], target_txt[:n_img]
wm_mask_img = wm0_mask[:n_img]

print(f"{font_name}-{case} {len(input_img)} examples.")

shape = tuple([batch_size] + list(input_img.shape[1:]))
...
with graph.as_default():
    ...
    new_vars = [x for x in end_vars if
                x.name not in start_vars]  # 优化在 optimizer 和 train 里的变量
...
with open(f'attack_result/wm_opt/{title}.pkl', 'wb') as f:
